...
export default function NoticeHomeScreen() {
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <ScrollView
        className="flex-1"
        contentContainerClassName="gap-16 px-[17px] pt-16 pb-32"
        showsVerticalScrollIndicator={false}
      >
        <Text className="text-[24px] leading-[32px]" weight="semibold" color="strong">
          놓치면 안 되는{'\n'}학교 소식
        </Text>

        <View className="w-full gap-10 overflow-hidden rounded-xl bg-primary-light p-20 shadow-lg">
          <View className="self-start rounded-full bg-background px-12 py-4">
            <Text size="caption" weight="semibold" color="primary">
              중요
            </Text>
          </View>
          <Text weight="semibold" className="text-[19px] leading-[26px] text-primary-dark">
            2024학년도 외국인 유학생{'\n'}건강보험 가입 및 갱신 안내
          </Text>
          <View className="flex-row items-center gap-8 pt-8 opacity-80">
          ...
          <Image
            source={shieldIcon}
            style={{ width: 80, height: 106, position: 'absolute', right: -24, bottom: -24 }}
          />
        </View>
        ...
        <View className="gap-16">
          <View className="flex-row items-center justify-between">
            ...
            <Pressable
              ...
              onPress={() => router.push('/notice/all')}
            >
              <Text size="caption" weight="semibold" color="primary">
                전체보기
              </Text>
          ...
          <View className="gap-12">
            {RECENT_NOTICES.map((notice) => (
              <Pressable
                key={notice.id}
                className="w-full gap-4 rounded-md border border-border-strong bg-background py-[17px] pl-[17px] pr-[14px] shadow-sm"
              ...
              >
                ...
                <View className="flex-row items-center gap-8 pt-4">
                  {!notice.read ? <View className="h-8 w-8 rounded-full bg-primary-light" /> : null}
                  <Text
                    className="flex-1 text-[15px] leading-[20px]"
                    weight="medium"
                    color={notice.read ? 'muted' : 'strong'}
                  >
                    {notice.title}
                  </Text>
                </View>
